...
確實，在 [`safe`package中](https://hackage.haskell.org/package/safe) 定義了這樣一個函數。
...
您可能會認為這樣做僅適用於沒有像您這樣的超級天才編碼的傢伙。 當然，您絕不會犯任何錯誤，例如將空列表傳遞給只需要非空函數的函數。 對？ 好吧，肯定有一個障礙，但這不是您的想法。