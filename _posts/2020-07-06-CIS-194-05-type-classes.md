...
請記住，這 `a` 是一個*類型變量*，可以代表任何類型。這種類型的函數有哪些？
...
事實證明這是行不通。語法有效，至少，但沒有類型檢查。我們收到以下錯誤消息：
...
作為 *承諾*，與這種類型的函數將工作不管是什麼類型的調用者進行選擇
...
這種類型的多態被稱為 *參數多態* 。我們說像這樣的函數在類型中 `f :: a -> a -> a` 是 *參數化* 的 `a`。在這裡，“參數”只是“對於呼叫者選擇的任何類型都可以統一工作”的幻想術語。在 Java 中，這種類型的多態性是由*泛型*提供的（您猜對了，它受 Haskell 的啟發：Haskell的原始設計師之一 [Philip Wadler](http://homepages.inf.ed.ac.uk/wadler/) 後來成為Java泛型開發的主要參與者之一）。
...
其實不是！實際上，我們 *可以* 使用 Haskell 根據類型決定要做什麼，只是不像我們以前想像的那樣。讓我們先來看一下的類型 `(+)`：
...
`Num`，`Eq`，`Ord`, 和 `Show` 是*類型類*，和我們說 `(==)`，`(<)` 和 `(+)` 有 “類型類多態(type-class polymorphic)”。直觀地講，類型類對應於為其定義了某些操作 *的類型集*，並且類型類多態函數僅適用於作為所討論類型類實例的類型。作為示例，讓我們詳細研究 `Eq` 類型類。
